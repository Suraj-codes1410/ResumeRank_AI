...
describe('CandidateDetailPage Tests', () => {
  ...
  it('renders parseError and retry button for a failed candidate', async () => {
    ...
    expect(screen.getByTestId('parse-error')).toHaveTextContent('Failed to extract text from PDF.');
    ...
    // Click retry
    vi.mocked(apiClient.post).mockResolvedValueOnce({ data: { id: 'new-candidate-456' } });
  ...
  it('changing pipeline status calls the PATCH endpoint with the correct payload', async () => {
    ...
    vi.mocked(apiClient.patch).mockResolvedValueOnce({ data: { ...mockCandidate, pipelineStatus: 'SHORTLISTED' } });
    ...
    // Select new status
    const statusSelect = screen.getByTestId('pipeline-status-dropdown') as HTMLSelectElement;
