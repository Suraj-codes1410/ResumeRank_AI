...
export default function CandidateDetailPage({
  ...
  const {
    ...
    refetchInterval: (query) => {
      ...
      return candidate.resumeStatus === "PENDING" ||
        candidate.resumeStatus === "PARSING"
        ? 3000
        : false;
    },
  });
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-brand-text-secondary max-w-md mx-auto">
                  The candidate you are looking for does not exist, or you do
                  not have permission to view this details record.
                </p>
              </div>
              <Link
                href="/job-postings"
          ...
          {status === "error" && !is404 && (
            <div className="flex flex-col items-center justify-center py-12 px-4 bg-brand-surface border border-brand-border rounded-xl max-w-md mx-auto text-center space-y-4">
              ...
              <p className="text-sm text-brand-text-secondary">
                We encountered an error loading candidate details. Please check
                your connection.
              </p>
              <button
                onClick={() => refetch()}
          ...
          {status === "success" && candidate && (
            <div className="space-y-8" data-testid="success-state">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
                  <div className="space-y-2">
                    <div className="flex flex-wrap items-center gap-3">
                      <h1
                        className={`text-2xl sm:text-3xl font-medium tracking-tight text-brand-text-primary ${fraunces.className}`}
                      >
                        {candidate.name || "Extraction pending"}
                      </h1>
                      <span
                        className={`text-[10px] sm:text-xs px-2.5 py-0.5 rounded-full font-bold border capitalize ${
                          candidate.pipelineStatus === "SHORTLISTED"
                            ...
                            : candidate.pipelineStatus === "REJECTED"
                              ...
                              : candidate.pipelineStatus === "REVIEWING"
                    ...
                    <div className="pt-2">
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 text-xs text-brand-accent hover:underline focus-visible:ring-2 focus-visible:ring-brand-accent focus-visible:ring-offset-2 focus-visible:ring-offset-brand-bg focus-visible:outline-none rounded"
                      >
                        <svg
                          className="h-4 w-4"
                          fill="none"
                  ...
                  <div className="flex flex-col gap-1.5 w-full sm:w-48">
                    <label
                      htmlFor="pipeline-status-select"
                    ...
                    <select
                      ...
                      className="w-full bg-brand-bg border border-brand-border rounded-lg px-3 py-2 text-xs transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent focus-visible:ring-offset-2 focus-visible:ring-offset-brand-bg text-brand-text-primary disabled:opacity-50"
              ...
              {(candidate.resumeStatus === "PENDING" ||
                candidate.resumeStatus === "PARSING") && (
                <div
                  className="bg-brand-surface border border-brand-border rounded-xl p-8 text-center space-y-4 shadow-xl"
                  data-testid="in-progress-state"
                >
                  <div className="inline-flex items-center justify-center p-3 bg-neutral-800/40 border border-brand-border rounded-full text-brand-accent animate-spin">
                    ...
                    >
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M4 4v5h.582m15.356 2A8.001 8.001 0 1121.21 19l-1.272-1.272"
                      />
                    </svg>
                  </div>
                  <h3
                    className={`text-xl font-medium text-brand-text-primary ${fraunces.className}`}
                  >
                    Resume Evaluation in Progress
                  </h3>
                  <p className="text-sm text-brand-text-secondary max-w-md mx-auto">
                    We are currently extracting and ranking this
                    candidate&apos;s resume. This page will update automatically
              ...
              {candidate.resumeStatus === "FAILED" && (
                ...
                >
                  <div className="flex items-start gap-4 p-4 bg-rose-950/15 border border-rose-500/20 rounded-lg">
                    <svg
                      className="h-5 w-5 text-rose-400 mt-0.5 flex-shrink-0"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                    ...
                    <div className="space-y-1">
                      ...
                      <p
                        className="text-xs text-rose-300 leading-relaxed"
                        data-testid="parse-error"
                      >
                        Error:{" "}
                        {candidate.parseError ||
                          "An unknown error occurred during resume parsing and AI scoring."}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] text-brand-text-secondary italic">
                      * Initiates a new AI evaluation cycle under a new
                      screening record.
                    </span>
                  </div>
                </div>
                    ...
                    <div className="bg-brand-surface border border-brand-border rounded-xl p-6 shadow-xl text-center space-y-4">
                      ...
                      {candidate.overallScore !== null ? (
                        <div className="flex flex-col items-center justify-center space-y-2">
                          <div
                            className={`h-28 w-28 rounded-full border-4 flex flex-col items-center justify-center ${getScoreColorClass(candidate.overallScore)}`}
                          >
                      ...
                      {candidate.yearsExperienceDetected !== null && (
                        <div className="pt-2 border-t border-brand-border/40">
                          ...
                          <span className="text-xs font-semibold text-brand-text-primary">
                            {candidate.yearsExperienceDetected}{" "}
                            {candidate.yearsExperienceDetected === 1
                              ? "year"
                              : "years"}
                          </span>
                        </div>
                      ...
                      <div className="space-y-4" data-testid="score-breakdown">
                        ...
                        <div className="space-y-1.5">
                          <div className="flex justify-between text-xs font-medium">
                            ...
                            <span className="text-brand-text-primary">
                              {candidate.experienceScore ?? 0}/100
                            </span>
                          </div>
                          <div className="h-1.5 bg-neutral-800 rounded-full overflow-hidden">
                            <div
                              className="h-full bg-brand-accent transition-all duration-500"
                              style={{
                                width: `${candidate.experienceScore ?? 0}%`,
                              }}
                            />
                          </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                          <div className="flex justify-between text-xs font-medium">
                            ...
                            <span className="text-brand-text-primary">
                              {candidate.seniorityScore ?? 0}/100
                            </span>
                          </div>
                          <div className="h-1.5 bg-neutral-800 rounded-full overflow-hidden">
                            <div
                              className="h-full bg-brand-accent transition-all duration-500"
                              style={{
                                width: `${candidate.seniorityScore ?? 0}%`,
                              }}
                            />
                          </div>
                        </div>
              ...
              <div className="bg-brand-surface border border-brand-border rounded-xl shadow-xl overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center justify-between p-5 text-sm font-semibold text-brand-text-primary hover:bg-neutral-800/10 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent focus-visible:ring-offset-2 focus-visible:ring-offset-brand-bg"
                ...
                {isHistoryOpen && (
                  <div
                    ...
                    className="p-5 border-t border-brand-border/40 bg-brand-bg/20 space-y-4"
                    data-testid="status-history-content"
                  >
                        ...
                        {statusLogs.map((log) => {
                          ...
                          return (
                            <div
                              ...
                              data-testid="status-history-log-row"
                            >
                              ...
                              <div className="space-y-1">
                                <p className="text-xs text-brand-text-primary">
                                  ...
                                  to{" "}
                                  <span className="font-semibold text-brand-accent-secondary">
                                    {log.toStatus}
                                  </span>
                                </p>
              ...
              <div className="flex justify-start">
                <Link
                  ...
                  className="px-5 py-2.5 border border-brand-border text-brand-text-secondary rounded-lg text-xs font-semibold hover:border-brand-text-primary hover:text-brand-text-primary focus-visible:ring-2 focus-visible:ring-brand-accent focus-visible:ring-offset-2 focus-visible:ring-offset-brand-bg focus-visible:outline-none transition-all"
                >
