...
export default function JobPostingsPage() {
  ...
  return (
    <ProtectedRoute>
      <div
        className={`min-h-screen bg-brand-bg text-brand-text-primary px-4 py-8 sm:px-6 lg:px-8 font-sans ${inter.variable} ${fraunces.variable} ${jetbrainsMono.variable}`}
      >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-brand-border pb-6">
            <div>
              ...
              <p className="text-sm text-brand-text-secondary mt-1">
                Manage your active and archived job openings for resume
                screening.
              </p>
            </div>
            <div>
          ...
          {status === "success" && data.totalItems > 0 && (
            <div className="flex border-b border-brand-border/60 pb-3 gap-6 text-sm">
              ...
              >
                Active
                {statusFilter === "ACTIVE" && (
                  <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-brand-accent-secondary" />
                )}
              </button>
              <button
                onClick={() => {
              ...
              >
                ...
                {statusFilter === "ARCHIVED" && (
                  <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-brand-text-primary" />
                )}
              </button>
            </div>
          )}
                    ...
                    {job.requiredSkills.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 pt-2">
                        ...
                        {job.requiredSkills.length > 3 && (
                          <span
                            className={`text-xs px-2 py-0.5 text-brand-text-secondary font-mono ${jetbrainsMono.className}`}
                          >
                            +{job.requiredSkills.length - 3}
                          </span>
                        )}
