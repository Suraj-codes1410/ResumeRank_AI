...
def test_extract_text_missing_token():
    response = client.post(
        "/internal/extract-text", json={"fileUrl": "http://example.com/test.pdf"}
    )
...
def test_extract_text_valid_pdf(mock_get, mock_pdf_reader):
    ...
    mock_page.extract_text.return_value = (
        "This is a resume for a developer. Contact: candidate@example.com"
    )
...
def test_process_resume_success(
    mock_run_extraction, mock_run_scoring, mock_requests_post
):
...
@patch("main.run_scoring")
@patch("main.run_extraction")
def test_process_resume_extraction_failure(
    mock_run_extraction, mock_run_scoring, mock_requests_post
):
